...
require_once(t3lib_extMgm::extPath('fe_edit') . 'view/class.tx_feedit_tceforms.php');
...
class tx_feedit_editpanel {
	...
	/**
	 * fe_edit TS configuration from admPanel
	 * @var		array
	 */
	...
	public function init($conf) {
		...
		$this->modTSconfig = t3lib_BEfunc::getModTSconfig($GLOBALS['TSFE']->id,'tx_fe_edit');
	...
	public function editPanel($content, array $conf, $currentRecord='', array $dataArr=array(), $table='', $allow='', $newUID=0, array $hiddenFields=array()) {
		...
		$actionURL = (t3lib_div::_GP('eID') == 'fe_edit') ? '/' : htmlspecialchars(t3lib_div::getIndpEnv('REQUEST_URI'));
		...
		if (($theCmd != 'edit') && ($theCmd != 'new')) {
			...
			$hiddenFieldString = '';
			if (is_array($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['typo3/sysext/fe_edit/view/class.tx_feedit_editpanel.php']['addHiddenFields'])) {
				foreach  ($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['typo3/sysext/fe_edit/view/class.tx_feedit_editpanel.php']['addHiddenFields'] as $classRef) {
	...
	protected function formsOnPageForm($markerArray, $table, $dataArr=array(), $currentRecord='', $conf='', $content='', $theCmd='edit', $newUID='', $fieldList='') {
		...
		tx_feedit_tceforms::changeTCAforFormsOnPage($GLOBALS['TCA'],$table);
		...
		$this->init($conf);
		$tceforms = t3lib_div::makeInstance('tx_feedit_tceforms');
	...
	protected function addIncludes() {
		...
		$GLOBALS['TSFE']->additionalHeaderData['fe_edit_inc'] .= '<script type="text/javascript" src="' . t3lib_extMgm::siteRelPath('fe_edit') . 'res/js/feEdit.js"></script>';
		...
		$GLOBALS['TSFE']->additionalHeaderData['fe_formsOnPage.css'] = '<link type="text/css" rel="stylesheet" href="typo3/sysext/fe_edit/res/css/fe_formsOnPage.css" />';
		...
		*/
		...
		if (is_array($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['typo3/sysext/fe_edit/view/class.tx_feedit_editpanel.php']['addIncludes'])) {
			foreach  ($GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['typo3/sysext/fe_edit/view/class.tx_feedit_editpanel.php']['addIncludes'] as $classRef) {
				$hookObj= &t3lib_div::getUserObj($classRef);
	...
	*/
	protected function addFormIncludes($tceforms=0) {
		...
		$incJS .= '<script type="text/javascript" src="' . t3lib_div::getIndpEnv('TYPO3_SITE_URL') . t3lib_extMgm::siteRelPath('fe_edit') . 'res/js/getDynTabMenuJScode.js"></script>';

			// forms on page CSS
		$cssfile = $this->modTSconfig['properties']['skin.']['cssFormFile'];
		$cssFormFile =  $cssfile ? $cssfile : t3lib_extMgm::siteRelPath('fe_edit') . 'res/css/fe_formsOnPage.css';
		...
		$incJS .= '<script type="text/javascript" src="' . t3lib_div::getIndpEnv('TYPO3_SITE_URL') . t3lib_extMgm::siteRelPath('fe_edit') . 'res/js/fe_logout_timer.js"></script>';

		if (t3lib_div::_GP('eID') != 'fe_edit') {
